	...
	<table class="table">
		<thead class="thead" data-dir={sort_dir}>
			<tr>
				...
				<th class="sort"
					class:active={sort_prop === 'hp'}
					on:click={() => sort_by_prop('hp')}
				>氣血</th>

				<th class="sort"
					class:active={sort_prop === 'atk'}
					on:click={() => sort_by_prop('atk')}
				>物攻</th>

				<th class="sort"
					class:active={sort_prop === 'matk'}
					on:click={() => sort_by_prop('matk')}
				>法攻</th>

				<th class="sort"
					class:active={sort_prop === 'def'}
					on:click={() => sort_by_prop('def')}
				>物防</th>

				<th class="sort"
					class:active={sort_prop === 'mdef'}
					on:click={() => sort_by_prop('mdef')}
				>法防</th>
		...
		<tbody class="tbody">
			{#each data as item}
				<tr style="--hp:{item.hp};--atk:{item.atk};--matk:{item.matk};--def:{item.def};--mdef:{item.mdef}">
					<td>
...
<style>
	...
	td, th {
		display: flex;
		text-align: inherit;
		border-bottom: 1px dotted rgba(var(--main-color-rgb), .3);
		place-items: center;
		padding-top: 0.25em;
