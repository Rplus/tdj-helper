...
let gsheet_url = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vThfydM3yjBOBsKXVmXDZo6Mr6iHiQO0di-C_J5Ct6bQ1weTorkbV8TOZ7wz0KmAPEk2qMj6zCqHz_J/pubhtml#';
...
onMount(() => {
	...
	caption.innerHTML = [...new Set([d1, date])]
	  .map(d => `<a href="${link('/naihe')}?date=${d}" data-target="${date === d}" data-sveltekit-reload>${d}🔗</a>`)
	  .join('');
	...
	function markDupOptions(argument) {
	  ...
	  updateStyle(
	  	values
	  	  ...
	  	  .join(',') + '{ text-decoration: line-through; opacity: .5; }'
  	);
	};
