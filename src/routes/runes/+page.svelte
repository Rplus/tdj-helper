...
import { clamp, getProp, genProps, cloneObj, BreakPoints, RockTypes, DefailtItem } from './u.js';
...
let refs = [
	{
		title: '計分標準refs: 魂石选择终极攻略 | NGA玩家社区',
		link: 'https://bbs.nga.cn/read.php?tid=26383169&rand=456',
		target: '_blank',
	},
];
</script>

<Header title="絕品魂石 評估" />
...
<div class="workspace">
	...
	>
		<fieldset>
			<legend>
				絕‧魂石孔位
				<input class="title" type="search" list="rock-types" bind:value={title} />
		...
		<fieldset class="">
			...
			<div class="flex jc-sb form-btns">
				...
				/>
				<input type="submit" value="生成上下限" on:click|preventDefault={saveMMM} />
...
.workspace {
	display: grid;
	grid-template: 'main aside' 1fr / 1fr 1fr;
...
.output {
	...
	background-image: linear-gradient(to right, #0001, #666a),
		linear-gradient(to right, #3331 calc(100% * var(--break-point) / var(--max)), #aaa1 0);
	background-size:
		calc(100% * var(--score) / var(--max)) 5px,
		100% 100%;
