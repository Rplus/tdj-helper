...
<details open class="aside">
	<summary> 魂石紀錄: </summary>

	<div class="ul">
		{#each $savedUrl as url (url)}
			<div class="li">
				<button class="btn" on:click|preventDefault={() => historeUrls.remove(url)}>⨯</button>

				<span
					tabindex="0"
					role="button"
					aria-hidden="true"
					on:click|preventDefault={() => applyUrl(url)}>{@html itemTitle(url)}</span
				>
			</div>
		{/each}
	</div>
