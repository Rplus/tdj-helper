...
function handle_error() {
	...
	try {
		...
		if (err_count === 0) {
			err_count = 1;
			const qs = parsed_url.searchParams;
			const url_param = qs.get('url');
			const is_default = (qs.get('default') === '1');

			// console.log(123, is_default);

			if (is_default && url_param) {
				const tw_path_prefix = '/media/pictures/tdj/';
				const tw_url = new URL(url_param);

				if (tw_url.pathname.startsWith(tw_path_prefix)) {
					const cn_domain = 'https://media.zlongame.com';
					const cn_pathname = tw_url.pathname.replace(
						tw_path_prefix,
						'/media/news/cn/tdj/'
					);
